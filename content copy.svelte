<script context="module" lang="ts">
  ...
  const breakPointToolRemoveResizerElement = function () {
    if (document.getElementById(resizerElementId)) {
      document
        .getElementById(resizerElementId)
  ...
  function handleResize(event) {
    ...
    responsiveSizeEl.className =
      "fixed top-0 ease-out duration-300 flex -translate-y-full items-center w-auto px-2 py-1 text-black text-xs dark:text-gray-200 -translate-x-1/2 border rounded-full border-neutral-200/50 dark:border-neutral-700/70 shadow-3xl backdrop-blur-lg bg-white/80 dark:bg-neutral-900 left-1/2";
    ...
    document.getElementById("currentWidth").innerText = currentWidth.toString();
...
{#key { handleResize }}
  {#if true}
    <div
      class="fixed z-[9999] top-0 ease-out duration-300 flex -translate-y-full items-center w-auto px-2 py-1 text-black text-xs dark:text-gray-200 -translate-x-1/2 border rounded-full border-neutral-200/50 dark:border-neutral-700/70 shadow-3xl backdrop-blur-lg bg-white/80 dark:bg-neutral-900 left-1/2"
      id={resizerElementId}
    >
      <div class="px-1.5 py-px font-bold bg-blue-500 rounded-full">
        <span class="inline-block sm:hidden">XS</span>
      ...
      <span class="px-2 py-1 text-black dark:text-white"
        ><span id="currentWidth">${currentWidth}</span>px</span
      >
      <span
        id="resizerElementCloseButton"
        class="flex items-center justify-center w-5 h-5 text-gray-500 rounded-full cursor-pointer dark:text-neutral-300 bg-neutral-200/60 dark:bg-neutral-700/80 dark:hover:bg-neutral-700 hover:bg-neutral-200"
      >
        <svg
          class="w-3 h-3"
